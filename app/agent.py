...
"""Blackjack Strategy Tutor: Multi-Agent game and tutoring system."""
...
def emit_observability_log(event_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    """Emits structured JSON observability log for Intents and Outcomes."""
    record = {
...
def execute_player_action(action: str, tool_context: ToolContext) -> dict[str, Any]:
    ...
    # Log the Intent in structured JSON format
    emit_observability_log(
        "player_intent",
        {
            "intent": action.lower(),
    ...
    if action.lower() == "hit":
        ...
        # Log the Outcome in structured JSON format
        emit_observability_log(
            "player_outcome",
            {
                "action": "hit",
...
6. Display current bankroll and offer the next hand.
